export default {
  ...
  // OAuth canonical scope display labels (v2). Keep aligned with backend
  // service.ScopeDisplay(scope, 'zh'). Each block has `name` (chip text) +
  // `description` (tooltip / secondary text). NOTE: locale strings here MUST
  // NOT contain a bare `@` because vue-i18n parses `@:foo` and `@.lower:foo`
  // as linked-message references, which would break the entire vendor-i18n
  // chunk. The current values use `/` instead.
  oauthScopes: {
    'profile:read': {
      name: '查看您的档案（用户名、头像）',
  ...
  admin: {
    ...
    groups: {
      ...
      openaiMessages: {
        ...
        allowDispatchHint: 'OpenAI 分组默认可通过 /v1/messages 调度；启用此配置面板可自定义 Claude 系列目标模型映射。',
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 Sakrylle API 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 Sakrylle API 用户登录',
      ...
      site: {
        ...
        siteNamePlaceholder: 'Sakrylle API',
        siteSubtitle: '站点副标题',
      ...
      payment: {
        ...
        balanceRechargeMultiplierHint: '用户每支付 1 元可获得多少元余额（数值即￥，建议保持 1.0）',
        balanceRechargePreview: '预览：支付 1 元 → 余额 ￥{usd}',
      ...
      smtp: {
        ...
        fromName: '发件人名称',
        fromNamePlaceholder: 'Sakrylle API',
        useTls: '使用 TLS',
