...
function localeLeaves(value: unknown, prefix = ''): { key: string; value: unknown }[] {
  ...
  return Object.entries(value as LocaleValue).flatMap(([key, child]) => {
    ...
    return localeLeaves(child, path)
  })
}
...
describe('locale key completeness', () => {
  const enKeys = new Set(localeLeaves(en).map(leaf => leaf.key))
  const zhKeys = new Set(localeLeaves(zh).map(leaf => leaf.key))
  ...
  it('contains a non-empty message for every locale leaf', () => {
    for (const [locale, messages] of Object.entries({ en, zh })) {
      // Keep the value with its path: OAuth scope names contain literal dots.
      const emptyKeys = localeLeaves(messages)
