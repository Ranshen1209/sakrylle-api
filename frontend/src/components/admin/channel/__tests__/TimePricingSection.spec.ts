...
describe('TimePricingSection', () => {
  ...
  it('offers every-day and weekdays-only scopes', async () => {
    ...
    expect(selects[1].props('options')).toEqual([
      { value: false, label: 'admin.channels.form.recurringEveryDay' },
      { value: true, label: 'admin.channels.form.recurringWeekdaysOnly' },
    ])
  ...
  it('removes without mutating props', async () => {
    ...
    expect(removeButton.attributes('title')).toBe('admin.channels.form.removeRecurringTimePeriod')
    expect(removeButton.attributes('aria-label')).toBe('admin.channels.form.removeRecurringTimePeriod')
