    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div class="min-w-0 flex-1 sm:max-w-2xl">
        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400">
          {{ t('admin.channels.form.recurringTimePricing') }}
        </label>
        <div class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
          <div class="min-w-0">
            <label class="block text-xs text-gray-400">
              {{ t('admin.channels.form.recurringTimezone') }}
            </label>
            <Select
              :model-value="modelValue.timezone"
              :options="timezoneOptions"
              :aria-label="t('admin.channels.form.recurringTimezone')"
          ...
          <div class="min-w-0">
            <label class="block text-xs text-gray-400">
              {{ t('admin.channels.form.recurringDayScope') }}
            </label>
            <Select
              :model-value="modelValue.weekdays_only"
              :options="dayScopeOptions"
              :aria-label="t('admin.channels.form.recurringDayScope')"
      ...
      <button
        ...
        @click="addPeriod"
      >
        + {{ t('admin.channels.form.addRecurringTimePeriod') }}
      </button>
    </div>
      ...
      >
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-start-${index}`" class="block text-xs text-gray-400">
            {{ t('admin.channels.form.recurringStartTime') }}
          </label>
          <input
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-end-${index}`" class="block text-xs text-gray-400">
            {{ t('admin.channels.form.recurringEndTime') }}
          </label>
          <input
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-multiplier-${index}`" class="block text-xs text-gray-400">
            {{ t('admin.channels.form.recurringMultiplier') }}
          </label>
          <input
        ...
        <button
          ...
          :title="t('admin.channels.form.removeRecurringTimePeriod')"
          :aria-label="t('admin.channels.form.removeRecurringTimePeriod')"
...
const dayScopeOptions = computed(() => [
  { value: false, label: t('admin.channels.form.recurringEveryDay') },
  { value: true, label: t('admin.channels.form.recurringWeekdaysOnly') },
])
