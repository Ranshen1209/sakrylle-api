...
function timePricingValidationMessage(t: TranslateFn, key: string): string {
  return t(`admin.channels.recurringTimePricingValidation.${key}`)
}

export function validateTimePricingCompatibility(
  versions: PricingTimeVersionFormEntry[] | undefined,
  recurring: TimePricingFormEntry | null | undefined,
  t: TranslateFn,
): string | null {
  if ((versions?.length ?? 0) > 0 && (recurring?.periods?.length ?? 0) > 0) {
    return t('admin.channels.timePricingValidation.pricingSystemsConflict')
  }
  return null
}
