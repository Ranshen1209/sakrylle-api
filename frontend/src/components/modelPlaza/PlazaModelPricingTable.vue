        ...
        >
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              ...
              <template v-else>{{ paidPerMillion(displayedTimePrice(m, 'input_price'), period) }}</template>
            </td>
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                <div
              ...
              <template v-else>{{ paidPerMillion(displayedTimePrice(m, 'output_price'), period) }}</template>
            </td>
            <td class="pz-cell px-3 py-2.5 align-middle">
              ...
              >
                <div>
                  ...
                  {{ paidPerMillion(displayedTimePrice(m, 'cache_write_price'), period) }}
                  <template v-if="displayedTimePrice(m, 'cache_write_1h_price') != null">
                    <span class="font-sans font-normal text-gray-400 dark:text-dark-500"> (1h </span>{{ paidPerMillion(displayedTimePrice(m, 'cache_write_1h_price'), period) }}<span class="font-sans font-normal text-gray-400 dark:text-dark-500">)</span>
                  </template>
                </div>
                <div>
                  <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheRead') }}</span>
                  {{ paidPerMillion(displayedTimePrice(m, 'cache_read_price'), period) }}
                </div>
              </div>
              <span v-else class="text-gray-400 dark:text-dark-500">-</span>
            </td>
...
import type { ModelPlazaPricingMode, PlazaModel, PlazaTimePricingPeriod } from '@/api/modelPlaza'
...
function hasCachePricing(m: PlazaModel): boolean {
  return displayedTimePrice(m, 'cache_write_price') != null || displayedTimePrice(m, 'cache_write_1h_price') != null || displayedTimePrice(m, 'cache_read_price') != null
}
