...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'grok' | 'deepseek'
...
const BADGE: Record<Platform, string> = {
  ...
  deepseek: 'bg-indigo-500/10 text-indigo-600 border-indigo-500/30 dark:text-indigo-400',
}
...
const BADGE_LIGHT: Record<Platform, string> = {
  ...
  deepseek: 'bg-indigo-500/10 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300',
}
...
const ICON: Record<Platform, string> = {
  ...
  deepseek: 'text-indigo-500 dark:text-indigo-400',
}
...
const BUTTON: Record<Platform, string> = {
  ...
  deepseek: 'bg-indigo-500 text-white hover:bg-indigo-600 active:bg-indigo-700 dark:bg-indigo-500/80 dark:hover:bg-indigo-500',
}
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'grok' || p === 'deepseek'
}
