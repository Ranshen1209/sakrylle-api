    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.qqEmailWarning.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.qqEmailWarning.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.qqEmailWarning.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.qqEmailWarning.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.qq_email_warning_enabled" />
            </div>
          </div>
        </div>
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "Sakrylle API") +
                        " 100 " +
...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/Ranshen1209/sub2api/blob/main/docs/PAYMENT_CN.md"
    : "https://github.com/Ranshen1209/sub2api/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/Ranshen1209/sub2api/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/Ranshen1209/sub2api/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
...
const form = reactive<SettingsForm>({
  ...
  // QQ Email Warning feature switch
  qq_email_warning_enabled: true,
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      // QQ Email Warning feature switch
      qq_email_warning_enabled: form.qq_email_warning_enabled,
      // Affiliate (邀请返利) feature switch
...
.settings-tab-active::after {
  ...
  content: "";
  background: linear-gradient(90deg, #9181bd, #0ea5e9);
}
