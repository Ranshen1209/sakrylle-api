...
Subdomains generally point A records to `154.36.159.42` and are DNS-only, except `sub.sakrylle.com`.
...
`api.sakrylle.com` and the origin server are not blocked. Failed connections never reach the server.
...
Mitigation applied: `sub.sakrylle.com` is orange-cloud proxied. This is the only exception to the DNS-only policy.
...
Full direct-access fix requires DoH plus ECH. Until then, mainland users need a proxy.
...
To revert orange cloud, patch the `sub` A record with `proxied:false`.
