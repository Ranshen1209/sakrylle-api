...
func TestGatewayModels_NoConfigurationReturnsEmpty(t *testing.T) {
	...
	groupID := int64(99)
	h := newGatewayModelsHandlerForTest(&gatewayModelsAccountRepoStub{byGroup: map[int64][]service.Account{groupID: {{ID: 1, Platform: service.PlatformComposite, Credentials: map[string]any{}}}}})
...
// Composite manifests aggregate administrator-configured models only.
...
func TestGatewayModels_UnmappedOpenAIAccountsDoNotExposeDefaults(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{
			name:     "unmapped parent and Spark shadow expose explicit mappings only",
			accounts: accounts,
			want:     []string{sparkModel, alias},
		},
		{
			name:     "unmapped API key account does not contribute defaults",
			accounts: append([]service.Account{{ID: 4, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey}}, accounts[1:]...),
			want:     []string{sparkModel, alias},
		},
		{
			name:     "unmapped accounts alone return an empty catalogue",
			accounts: accounts[:1],
			want:     []string{},
		},
		{
			name:     "allowlist can select configured models only",
			accounts: accounts,
...
func TestGatewayCodexModels_CompositeAnthropicDoesNotAdvertiseAntigravityDefaults(t *testing.T) {
	...
	// Gemini 2.5 Flash is a regular Gemini fallback model; Antigravity-only
	// defaults such as the 3.6 reasoning variants must remain hidden when no
	// Antigravity account is attached.
	require.NotContains(t, slugs, "gemini-3.6-flash")
}
...
func TestGatewayModels_CompositeUnmappedAccountsDoNotAddGenericPlatformDefaults(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// Grok's account-level effective mapping is retained; unlike a generic
	// platform fallback, it is the mapping used by that linked account.
...
func TestGatewayModels_AnthropicCustomModelsListDoesNotAddOAuthDefaults(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestGatewayModels_AnthropicCustomModelsListWithoutMappingsStaysEmpty(t *testing.T) {
	gin.SetMode(gin.TestMode)
