...
func TestVideoPromptGuardRunsBeforeBillingOrAccountSelection(t *testing.T) {
	gin.SetMode(gin.TestMode)
	engine := blockingHandlerPromptEngine()
	...
	router := gin.New()
	router.Use(securityAuditMediaTestMiddleware)
	router.POST("/v1/videos", h.Videos)
	request := httptest.NewRequest(http.MethodPost, "/v1/videos", strings.NewReader(`{"model":"agnes-video-v2.0","prompt":"blocked video prompt"}`))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()

	require.NotPanics(t, func() { router.ServeHTTP(recorder, request) }, "downstream nil repositories must remain untouched after a blocking decision")
	require.Equal(t, http.StatusForbidden, recorder.Code)
	require.Contains(t, recorder.Body.String(), securityaudit.ErrorCodeBlocked)
	evaluated, _, requests := engine.snapshot()
	require.Equal(t, 1, evaluated)
	require.Len(t, requests, 1)
	require.Equal(t, service.ContentModerationProtocolOpenAIImages, requests[0].Protocol)
