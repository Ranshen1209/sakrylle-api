...
func TestAllowsOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	...
	otherOff := &service.APIKey{Group: &service.Group{Platform: service.PlatformAnthropic, AllowMessagesDispatch: false}}
	require.False(t, allowsOpenAICompatibleMessagesDispatch(nil, otherOff))
	otherOn := &service.APIKey{Group: &service.Group{Platform: service.PlatformAnthropic, AllowMessagesDispatch: true}}
	require.True(t, allowsOpenAICompatibleMessagesDispatch(nil, otherOn))
}

func TestAllowsOpenAICompatibleMessagesDispatch_CompositeResolvedTargets(t *testing.T) {
	...
	for _, model := range []string{"grok-4.3", "kimi-k2-thinking", "glm-5.2", "deepseek-v3.2"} {
		...
		require.True(t, allowsOpenAICompatibleMessagesDispatch(c, apiKey), "model=%s", model)
	}

	// 解析到 openai 目标：受 composite 分组自身开关控制。
	c, apiKey := newCompositeCtx("gpt-5.5", false)
	require.False(t, allowsOpenAICompatibleMessagesDispatch(c, apiKey))
	c, apiKey = newCompositeCtx("gpt-5.5", true)
	require.True(t, allowsOpenAICompatibleMessagesDispatch(c, apiKey))

	// 未解析出目标平台：保持拒绝，不放宽。
	cNone, _ := gin.CreateTestContext(httptest.NewRecorder())
	cNone.Request = httptest.NewRequest("POST", "/v1/messages", nil)
	require.False(t, allowsOpenAICompatibleMessagesDispatch(cNone,
