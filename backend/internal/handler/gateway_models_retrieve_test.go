		...
		for _, mapped := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				if !mapped && platform != service.PlatformGrok {
					require.Empty(t, catalog.Data, "unconfigured accounts must not expose defaults")
					require.Equal(t, http.StatusNotFound, requestModelForTest(h, group, "gpt-5.6-sol", "").Code)
					return
				}
