	...
	return func(c *gin.Context) {
		...
		if !skipBilling {
			...
			case service.StatusAPIKeyQuotaExhausted:
				abortWithAPIKeyQuotaAuthError(c, isOAuth)
				return
...
func abortWithAPIKeyQuotaAuthError(c *gin.Context, isOAuth bool) {
	if isOAuth {
		abortAuthError(c, true, http.StatusTooManyRequests, "API_KEY_QUOTA_EXHAUSTED", "insufficient_quota", "API key 额度已用完")
		return
	}
