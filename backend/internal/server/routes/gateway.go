...
func RegisterGatewayRoutes(
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, requireOAuthScope, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, rejectOAuthUnlisted)
	{
	...
	rootRoute(http.MethodPost, "/files", bodyLimit, filesHandler)
	rootRoute(http.MethodGet, "/files", bodyLimit, filesHandler)
	rootRoute(http.MethodGet, "/files/:file_id", bodyLimit, filesHandler)
	rootRoute(http.MethodDelete, "/files/:file_id", bodyLimit, filesHandler)
	...
	rootRoute(http.MethodPost, "/videos", bodyLimit, videosPostHandler)
