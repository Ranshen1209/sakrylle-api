...
func TestBuildIDTokenClaims_RequiredClaims(t *testing.T) {
	...
	claims, err := BuildIDTokenClaims(testIssuer, "client-x", baseUser(), []string{"openid"}, "", now, time.Hour)
	if err != nil {
	...
	if claims["aud"] != "client-x" {
		t.Errorf("aud = %v, want client-x", claims["aud"])
	}
...
func TestBuildIDTokenClaims_ScopeGating(t *testing.T) {
	...
	c, _ := BuildIDTokenClaims(testIssuer, "c", baseUser(), []string{"openid"}, "", now, time.Hour)
	...
	c2, _ := BuildIDTokenClaims(testIssuer, "c", baseUser(), []string{"openid", "profile", "email"}, "", now, time.Hour)
...
func TestBuildIDTokenClaims_NonceRoundTrip(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	withNonce, _ := BuildIDTokenClaims(testIssuer, "c", baseUser(), []string{"openid"}, "n-123", now, time.Hour)
	if withNonce["nonce"] != "n-123" {
		t.Errorf("nonce must be echoed; got %v", withNonce["nonce"])
	}
	noNonce, _ := BuildIDTokenClaims(testIssuer, "c", baseUser(), []string{"openid"}, "", now, time.Hour)
...
func TestBuildIDTokenClaims_Validation(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	if _, err := BuildIDTokenClaims("", "c", baseUser(), []string{"openid"}, "", now, time.Hour); err == nil {
		t.Error("empty issuer must error")
	}
	if _, err := BuildIDTokenClaims(testIssuer, "", baseUser(), []string{"openid"}, "", now, time.Hour); err == nil {
		t.Error("empty client_id must error")
	}
	if _, err := BuildIDTokenClaims(testIssuer, "c", OIDCUserClaims{UserID: 0}, []string{"openid"}, "", now, time.Hour); err == nil {
...
func TestBuildIDTokenClaims_NoBusinessData(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	c, err := BuildIDTokenClaims(testIssuer, "c", baseUser(), []string{"openid", "profile", "email"}, "n", now, time.Hour)
	if err != nil {
		t.Fatal(err)
	}
