...
func (s *OpenAIGatewayService) ForwardGrokMedia(
	...
	if endpoint == GrokMediaEndpointImagesGenerations || endpoint == GrokMediaEndpointImagesEdits {
		if countOpenAIResponseImageOutputsFromJSONBytes(responseBody) <= 0 {
			setOpsUpstreamError(c, http.StatusBadGateway, "xAI upstream returned no image output", truncateString(string(responseBody), 512))
	...
	if endpoint == GrokMediaEndpointVideoStatus {
		responseBody = rewriteGrokMediaVideoContentURLs(
			responseBody,
			requestID,
